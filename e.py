		...
		for (markerCorner, markerID) in zip(corners, ids):
			...
			if(accumulator >= 10) : 
				if(direction_sum > 0) :
					...
					direct_str = "LEFT"
				send_str = "TURN," + direct_str 	# when Arduino gets TURN and LEFT or RIGHT messages, it moves accordingly. 
				ser.flush()
				ser.write(send_str.encode('utf-8'))
			elif(markerID == 1 and accumulator < 10) : 
				#Section to send dist and angle to Arduino
				dist_str = str(dist)
				angle_str = str(angle)
				send_str = dist_str + "," + angle_str + "\n"
				print(send_str)
				ser.flush()
				ser.write(send_str.encode('utf-8'))
